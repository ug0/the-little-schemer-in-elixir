defmodule Schemer.FriendsAndRelationsTest do
  use ExUnit.Case
  doctest Schemer.FriendsAndRelations
  ...
  test "subset" do
    assert subset?([5, :chicken, :wings], [5, :hamburgers, 2, :pieces, :fried, :chicken, :and, :light, :duckling, :wings])
    refute subset?([4, :pounds, :of, :horseradish], [:four, :pounds, :chicken, :and, 5, :ounces, :horseradish])
  end

  test "eqset?" do
    assert eqset?([6, :large, :chickens, :with, :wings], [6, :chickens, :with, :large, :wings])
    refute eqset?([6, :large, :chickens, :with, :wings, :of], [6, :chickens, :with, :large, :wings])
  end
  ...
  test "union" do
    assert union([:stewed, :tomatoes, :and, :macaroni, :casserole], [:macaroni, :and, :cheese]) == [:stewed, :tomatoes, :casserole, :macaroni, :and, :cheese]
  end
  ...
  test "a_pair?" do
    assert a_pair?([3, 7])
    assert a_pair?([[2], [:pair]])
    assert a_pair?([:full, [:house]])
  end
